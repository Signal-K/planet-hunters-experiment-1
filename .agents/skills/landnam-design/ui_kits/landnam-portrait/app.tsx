...
function LandnamApp(): React.ReactElement {
  ...
  const START: StartParams = (typeof window !== 'undefined' && window.LANDNAM_START) || {};

  const [screen, setScreen] = useState<ScreenId>(START.screen || 'build');
  const [built, setBuilt]   = useState<boolean>(START.built != null ? START.built : false);
  ...
  const [missionId, setMissionId] = useState<string | null>(START.missionId || null);
  const [targetId, setTargetId]   = useState<string | null>(START.targetId || null);
  const [rocket, setRocket]       = useState<RocketBuild>({ chassis: 'hull-mk1', propulsion: 'ion-a1', drill: 'hand-drill' });
  const [lastCargo, setLastCargo] = useState<Cargo | null>(START.cargo || null);
  const [menuOpen, setMenuOpen]   = useState<boolean>(!!START.menu);
  // ── Tutorial / onboarding state ──────────────────────────────────
  const [tutorial, setTutorial] = useState<boolean>(START.tutorial !== false);  // M1 coached run active
  const [doneSteps, setDoneSteps] = useState<DoneStepsMap>(START.doneSteps || {});   // step.id -> true
  const [subStep, setSubStep] = useState<number>(0);           // index within same-screen steps
  const [popup, setPopup] = useState<PopupKind | null>(START.popup || null);   // 'sr2' | 'freeops' | 'loan'
  const [buildGate, setBuildGate] = useState<boolean>(!!START.gate);  // Control Station gate
  ...
  const stepsHere: TutorialStep[] = tutorial ? TUT.M1_STEPS.filter(s => s.screen === screen && !doneSteps[s.id]) : [];
  const coach: TutorialStep | null = stepsHere[0] || null;
  ...
  function coachManualNext(): void {
    if (coach) setDoneSteps(d => ({ ...d, [coach.id]: true }));
  }

  const mission: Mission | null = missionId ? D.MISSIONS.find(m => m.id === missionId) || null : null;
  const target: Target | null  = targetId  ? D.TARGETS.find(t => t.id === targetId) || null   : null;

  function go(s: ScreenId): void { setScreen(s); }

  function onPickMission(id: string): void {
    setMissionId(id);
    setTargetId(null);
    go('targets');
  }

  function onPickTarget(id: string): void {
    setTargetId(id);
    // auto-suggest a starting build so the user sees a valid rocket immediately
    const targetForBuild = D.TARGETS.find(t => t.id === id);
    if (!targetForBuild) return;
    const next = D.suggestBuild({ mission, target: targetForBuild, level: player.level });
    setRocket(next);
    go('fab');
  }
  ...
  function onMiningDone(cargo: Cargo): void {
    setLastCargo(cargo);
    go('debrief');
  }

  function onDebriefDone(total: number, xp: number): void {
    setPlayer(p => ({
      ...p,
    ...
    <div style={{ width: '100%', height: '100%', position: 'relative', overflow: 'hidden', background: '#06090f' }}>
      ...
      {screen === 'fab' && mission && target && (
        <SP.FabScreen
          ...
          target={target}
          rocket={rocket}
          onChange={(slot: PartSlot, id: string) => setRocket(r => ({ ...r, [slot]: id }))}
...
function DevTutorialBar({ onReplay, onSR2, onFreeOps, onLoan, onGate }: DevTutorialBarProps) {
...
window.LandnamApp = LandnamApp as unknown as Window['LandnamApp'];
