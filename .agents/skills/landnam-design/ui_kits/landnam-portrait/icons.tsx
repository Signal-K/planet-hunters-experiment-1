...
const Stroke: React.FC<StrokeProps> = ({ d, c = [], size = 22, strokeWidth = 2, fill, children }) => (
...
function Planet({ id, size = 64 }: PlanetProps) {
  const s = size;
  const r = s / 2;
  // common: glowing disc with subtle highlight
  const baseLight = '#ffffff', baseShadow = '#000';
  const dot = (cx: number, cy: number, rr: number, c: string, op = 1) => <circle cx={cx} cy={cy} r={rr} fill={c} opacity={op} />;
  switch (id) {
    case 'mercury': return (
...
function ContractorBadge({ contractor, size = 44 }: ContractorBadgeProps) {
...
interface MineralGlyphProps {
  id: MineralId;
  size?: number;
}

function MineralGlyph({ id, size = 28 }: MineralGlyphProps) {
...
function Building({ kind, label, sub, status, onClick, locked, hot, w = 110, style }: BuildingProps) {
  return (
    <button onClick={!locked ? onClick : undefined}
      ...
      <div className="ln-building-art" style={{ width: w, height: w, position: 'relative', transition: 'transform 220ms cubic-bezier(.34,1.56,.64,1)', filter: 'drop-shadow(0 6px 10px rgba(0,0,0,0.45))' }}>
        <BuildingArt kind={kind} hot={hot} />
        {/* ground shadow ellipse */}
        <div style={{
          position: 'absolute', left: '50%', bottom: 2, transform: 'translateX(-50%)',
          width: w * 0.6, height: w * 0.12, borderRadius: '50%',
      ...
      <div style={{
        ...
        {sub && <span style={{
          width: 5, height: 5, borderRadius: 999, flex: '0 0 auto',
        ...
        {sub && <span style={{
          fontFamily: 'var(--ln-font-mono)', fontSize: 8,
...
function BuildingArt({ kind, hot }: BuildingArtProps) {
  switch (kind) {
...
function darken(hex: string, t: number): string {
  const c = hex.replace('#','');
  const r = parseInt(c.slice(0,2),16), g = parseInt(c.slice(2,4),16), b = parseInt(c.slice(4,6),16);
  const mix = (v: number) => Math.max(0, Math.round(v * (1 - t)));
  return '#' + [mix(r), mix(g), mix(b)].map(v => v.toString(16).padStart(2,'0')).join('');
}

window.LandnamIcons = { I, Planet, Sun, ContractorBadge, MineralGlyph, Building, BuildingArt, lighten, darken } as unknown as Window['LandnamIcons'];
