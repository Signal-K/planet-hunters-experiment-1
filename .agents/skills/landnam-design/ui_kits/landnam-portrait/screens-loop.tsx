...
interface SurfacePaletteShared {
  sky1: string;
  sky2: string;
  dist: string;
  ground: string;
...
function TransitScreen({ rocket, target, mission, onArrive, onBack }: TransitScreenProps) {
  ...
  const chassis = D().PARTS.chassis.find(p => p.id === rocket.chassis) as ChassisPart;
  const propulsion = D().PARTS.propulsion.find(p => p.id === rocket.propulsion) as PropulsionPart;

  // rocket position during ascent: starts low, rises to top of frame
  // during cruise: arc from top-left to target on right
  const W = 360, H = 320;
  let rx: number, ry: number, rAngle: number;
  if (t < 0.45) {
...
interface MiningScreenProps {
  ...
  target: Target;
  mission: Mission | null;
  onComplete: (cargo: Cargo) => void;
  onBack: () => void;
}

function MiningScreen({ rocket, target, mission, onComplete, onBack }: MiningScreenProps) {
  const { PARTS, MINERAL_META } = D();
  const { TopBar, PrimaryBtn, GhostBtn, StatusPill, Panel } = CH();
  const { MineralGlyph, I } = IC();

  const chassis = PARTS.chassis.find(p => p.id === rocket.chassis) as ChassisPart;
  const drill   = PARTS.drill.find(p => p.id === rocket.drill) as DrillPart;
  const cargoMax = chassis.cargo;

  // ── state ─────────────────────────────────────────────────────────
  const [cargo, setCargo]   = useState<Cargo>({});
  const [pops, setPops]     = useState<OrePop[]>([]);
  const [sparks, setSparks] = useState<Spark[]>([]);
  const [time, setTime]     = useState<number>(45);
  const [ores, setOres]     = useState<OreDeposit[]>(() => spawnOres(target, 6));
  const [beamTo, setBeamTo] = useState<BeamTarget | null>(null);  // {x, y, until}
  const [lunge, setLunge]   = useState<BeamTarget | null>(null);  // ore the rocket is diving toward
  const [scrollUnused] = useState<number>(0);

  const total = useMemo(() => Object.values(cargo).reduce((a: number, b) => a + (b as number), 0), [cargo]);
  ...
  function tapOre(o: OreDeposit): void {
    ...
    setLunge({ x: o.x, y: o.y });
    setTimeout(() => setLunge(null), 220);
    // sparks at impact
    const burst: Spark[] = Array.from({ length: 10 }).map(() => ({
      id: Math.random(),
  ...
  const req: Partial<Record<MineralId, number>> = mission ? mission.requires.minerals : {};
  const reqDone = Object.entries(req).every(([k, v]) => (cargo[k as MineralId] || 0) >= (v as number));
  const pct = (total / cargoMax) * 100;

  // surface palettes by target
  const PAL: Record<string, SurfacePaletteShared> = {
  ...
  const pal: SurfacePaletteShared = PAL[target.id] || PAL.mars;
  ...
  return (
    <div className="scene" style={{ width: '100%', height: '100%', position: 'relative', background: pal.sky1 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, paddingTop: 72, paddingBottom: 120, overflowY: 'auto' }}>
        ...
        <div style={{
          ...
          <svg width="100%" height="100%" viewBox={'0 0 ' + W + ' ' + H} preserveAspectRatio="none"
            ...
            {beamTo && (
              <g>
                <line x1={ROCKET_X + 4} y1={ROCKET_Y + 16} x2={beamTo.x} y2={beamTo.y} stroke="#CCE6FF" strokeWidth="7" opacity="0.35"/>
                <line x1={ROCKET_X + 4} y1={ROCKET_Y + 16} x2={beamTo.x} y2={beamTo.y} stroke="#9EDCFF" strokeWidth="3.5" opacity="0.7"/>
                <line x1={ROCKET_X + 4} y1={ROCKET_Y + 16} x2={beamTo.x} y2={beamTo.y} stroke="#fff" strokeWidth="1.4" opacity="1"/>
                <circle cx={beamTo.x} cy={beamTo.y} r="16" fill="url(#beamGrad)"/>
                <circle cx={beamTo.x} cy={beamTo.y} r="6"  fill="#fff" opacity="0.95"/>
              </g>
            )}
          </svg>
          ...
          {sparks.map(s => (
            <div key={s.id} style={{
              ...
              ['--sx' as string]: s.vx + 'px',
              ['--sy' as string]: s.vy + 'px',
              pointerEvents: 'none',
            } as React.CSSProperties}/>
          ))}

          {/* rocket sprite — hovers over the dig site, lunges toward tapped ore */}
          ...
          {/* ores — buried in the soil, static (tap to beam-mine them) */}
            ...
            return (
              <button key={o.id} onClick={() => tapOre(o)} disabled={full}
                style={{
                  ...
                  fontFamily: 'var(--ln-font-mono)', fontWeight: 800, fontSize: 11 + o.size * 2,
            ...
            <Panel accent={D().CONTRACTORS[mission.contractor].color}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {Object.entries(req).map(([k, v]) => {
                  const mineralId = k as MineralId;
                  const got = cargo[mineralId] || 0;
                  const meta = MINERAL_META[mineralId];
                  const reqVal = v as number;
                  const p = Math.min(100, (got / reqVal) * 100);
                  const done = got >= reqVal;
                  return (
                    <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <MineralGlyph id={mineralId} size={22}/>
                      ...
                      <span style={{ fontFamily: 'var(--ln-font-mono)', fontSize: 11, color: done ? '#39d36a' : '#a9b8ce', minWidth: 50, textAlign: 'right' }}>{got}/{reqVal}{done ? ' ✓' : ''}</span>
                    </div>
                  );
                })}
              </div>
      ...
      <div style={{
        ...
        <PrimaryBtn kind={reqDone ? 'green' : 'cyan'} disabled={false} onClick={() => onComplete(cargo)}>
          {full ? 'Cargo Full · Return →' : reqDone ? 'Mission Done · Return →' : total === 0 ? 'Return Empty →' : 'Return Now →'}
        </PrimaryBtn>
      </div>
...
// ─── parallax pieces ─────────────────────────────────────────────────
interface ParallaxPieceProps {
  w: number;
  h: number;
  pal: SurfacePaletteShared;
}

interface ForegroundProps extends ParallaxPieceProps {
  groundY: number;
}

function DistantRange({ w, h, pal }: ParallaxPieceProps) {
  // far range silhouette
  return (
...
function MidRange({ w, h, pal }: ParallaxPieceProps) {
  return (
...
function Foreground({ w, h, pal, groundY }: ForegroundProps) {
...
function spawnOres(target: Target, n: number, W = 360, H = 380): OreDeposit[] {
  const out: OreDeposit[] = [];
  const GROUND_Y = 210;
...
function DebriefScreen({ mission, target, cargo, onDone }: DebriefScreenProps) {
  ...
  const reqMet = mission ? Object.entries(mission.requires.minerals).every(([k, v]) => (cargo[k as MineralId] || 0) >= (v as number)) : false;
                ...
                {Object.entries(mission.requires.minerals).map(([k, v]) => {
                  ...
                  return (
                    <div key={k} style={{
                      ...
                      <MineralGlyph id={mineralId} size={18}/>
                      <span style={{ fontFamily: 'var(--ln-font-display)', fontSize: 11, fontWeight: 800, color: done ? '#39d36a' : '#ff8290' }}>{got}/{reqVal}</span>
                    </div>
                  );
                })}
              </div>
              ...
              {Object.entries(cargo).map(([k, v]) => {
                ...
                return (
                  <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <MineralGlyph id={mineralId} size={24}/>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontFamily: 'var(--ln-font-display)', fontWeight: 700, fontSize: 13, color: '#e6efff', letterSpacing: '0.04em' }}>{meta.name}</div>
                      <div style={{ fontFamily: 'var(--ln-font-mono)', fontSize: 10, color: '#7a8294', letterSpacing: '0.12em' }}>×{qty} @ ▲{meta.price}</div>
                    </div>
...
window.LandnamScreensLoop = { TransitScreen, MiningScreen, DebriefScreen } as unknown as Window['LandnamScreensLoop'];
