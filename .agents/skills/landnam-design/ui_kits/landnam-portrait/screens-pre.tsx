...
type BuildingId = 'satellite' | 'launchpad' | 'control' | string;

interface HubScreenProps {
  player: PlayerWithExtras;
  onGoBuilding: (id: BuildingId | string) => void;
  onNav: (id: string) => void;
}

interface HubBuildingSlot {
  id: string;
  kind: string;
  label: string;
  sub: string;
...
function HubScreen({ player, onGoBuilding, onNav }: HubScreenProps) {
  ...
  return (
    <div className="scene" style={{ width: '100%', height: '100%', position: 'relative', overflow: 'hidden' }}>
      ...
      <SceneBg image="../../assets/scenes/earth-day-sm.jpg" dim={0}/>
...
// ──────────────────────────────────────────────────────────────────────
interface HubTopHUDProps {
  player: Player;
}

function HubTopHUD({ player }: HubTopHUDProps) {
  return (
    <div style={{
...
function Cloud({ style, dur = '60s', delay = '0s' }: CloudProps) {
  return (
    <div style={{
      position: 'absolute',
...
function EmptyPlot({ w = 90, style, onClick }: EmptyPlotProps) {
  return (
...
function SoilCrossSection({ onMarket }: SoilCrossSectionProps) {
  return (
    <div style={{
...
function MissionBoardScreen({ onBack, onPick, player }: MissionBoardScreenProps) {
  ...
  return (
    <div className="scene" style={{ width: '100%', height: '100%', position: 'relative', background: '#06090f' }}>
      <SceneBg image="../../assets/scenes/earth-day-sm.jpg" dim={0.7}/>
              ...
              <button key={m.id}
                ...
                <Panel accent={accent} style={{ padding: 12 }}>
                  ...
                  <div style={{ marginTop: 10, display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
                    {(Object.entries(m.requires.minerals) as [MineralId, number][]).map(([k, v]) => {
...
function TargetPickerScreen({ mission, onBack, onPick }: TargetPickerScreenProps) {
  ...
  const [picked, setPicked] = useState<string | undefined>(compat.find(t => t.recommended)?.id || compat[0]?.id);
  ...
  function place(t: Target): { x: number; y: number } {
...
interface FabScreenProps {
  mission: Mission;
  target: Target;
  rocket: RocketBuild;
  onChange: (slot: PartSlot, id: string) => void;
  onSuggest: () => void;
  onLaunch: () => void;
  onBack: () => void;
}

function FabScreen({ mission, target, rocket, onChange, onSuggest, onLaunch, onBack }: FabScreenProps) {
        ...
        <div style={{ padding: '0 14px 12px' }}>
          <Panel accent={D().CONTRACTORS[mission.contractor].color} style={{ padding: 10 }}>
            ...
            <div style={{ marginTop: 8, display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {(Object.entries(mission.requires.minerals) as [MineralId, number][]).map(([k, v]) => (
...
type AnyPart = ChassisPart | PropulsionPart | DrillPart;
...
function SlotRow({ slot, label, picked, parts, onPick, accent, target, mission }: SlotRowProps) {
  const { Panel } = CH();
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 6, fontFamily: 'var(--ln-font-mono)', fontSize: 11 }}>
              {slot === 'chassis' && <>
                <Stat k="MASS" v={(picked as ChassisPart).mass + 'T'}/>
                <Stat k="CARGO" v={(picked as ChassisPart).cargo + 'U'} amber={!!mission && (picked as ChassisPart).cargo < mission.requires.cargo_min}/>
              </>}
              {slot === 'propulsion' && <>
                <Stat k="PWR" v={(picked as PropulsionPart).power + 'MW'}/>
                <Stat k="REACH" v={'≤O' + (picked as PropulsionPart).max_orbit} amber={!!target && (picked as PropulsionPart).max_orbit < target.orbit}/>
              </>}
              {slot === 'drill' && <>
                <Stat k="RATE" v={'×' + (picked as DrillPart).rate}/>
...
function Stat({ k, v, amber }: StatProps) {
  return (
...
function BuildPlaceScreen({ onPlaced, onBack }: BuildPlaceScreenProps) {
  const { TopBar, Panel, PrimaryBtn, GhostBtn, StatusPill, SceneBg } = CH();
  const { BuildingArt, I } = IC();
  const startParams = (typeof window !== 'undefined' && window.LANDNAM_START) as (StartParams & { buildPhase?: BuildPhase }) | undefined;
  const [phase, setPhase] = useState<BuildPhase>((startParams && startParams.buildPhase) || 'pick');
  const [picked, setPicked] = useState<string>('launchpad');
  const [cell, setCell] = useState<number | null>(null);

  const CATALOG: BuildCatalogItem[] = [
  ...
  const sel = CATALOG.find(c => c.id === picked) as BuildCatalogItem;
  ...
  return (
    <div className="scene" style={{ width: '100%', height: '100%', position: 'relative' }}>
      <SceneBg image="../../assets/scenes/earth-day-sm.jpg" dim={phase === 'place' ? 0.35 : 0.12}/>
...
} as unknown as Window['LandnamScreensPre'];

// ──────────────────────────────────────────────────────────────────────
// AmbientStars — 55 procedurally placed dots in the upper sky
// ──────────────────────────────────────────────────────────────────────
interface AmbientStarDot {
  x: number;
  y: number;
  r: number;
  o: number;
...
function AmbientStars() {
  const dots = React.useMemo<AmbientStarDot[]>(() => {
    const out: AmbientStarDot[] = [];
    let seed = 1337;
...
interface ProgressionCardProps {
  player: PlayerWithExtras;
  onGoBuilding: (id: string) => void;
  onNav: (id: string) => void;
}

interface ProgressionCardData {
  accent: string;
  eyebrow: string;
  title: string | number;
  cta: string;
...
function ProgressionCard({ player, onGoBuilding, onNav }: ProgressionCardProps) {
  let card: ProgressionCardData;
...
// ──────────────────────────────────────────────────────────────────────
type GameMenuTab = 'menu' | 'logbook' | 'discoveries';
...
function GameMenu({ player, onClose }: GameMenuProps) {
  const { Panel, StatusPill, IconBtn } = CH();
  const { MineralGlyph } = IC();
  const { MINERAL_META } = D();
  const [tab, setTab] = useState<GameMenuTab>('menu');

  const cargo: Cargo = player.stash || { iron: 24, silicon: 40, ice: 12 };
    ...
    <div style={{ position: 'absolute', inset: 0, zIndex: 60 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 6, padding: '0 16px 10px' }}>
          {([['menu','Menu'],['logbook','Logbook'],['discoveries','Discoveries']] as [GameMenuTab, string][]).map(([id, lbl]) => (
          ...
          {tab === 'menu' && <>
            ...
            <Panel accent="#3fa9ff">
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {(Object.entries(cargo) as [MineralId, number][]).map(([k, v]) => (
...
function MenuRow({ label, toggle, danger }: MenuRowProps) {
...
type GreeblyPos = 'tl' | 'tr' | 'bl' | 'br';

interface GreeblyProps {
  pos: GreeblyPos;
}

function Greebly({ pos }: GreeblyProps) {
...
function GalaxyScreen({ onBack, onPickStar }: GalaxyScreenProps) {
  const { STARS, STAR_LINKS } = D();
  const { TopBar, Panel, StatusPill, PrimaryBtn } = CH();

  const [picked, setPicked] = useState<string>('sol');
    ...
    <div className="scene" style={{ width: '100%', height: '100%', position: 'relative', background: '#05060d' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, paddingTop: 72, paddingBottom: 96, overflowY: 'auto' }}>
        ...
        <div style={{
          ...
          {/* stars */}
          {STARS.map(s => {
            const colors: Record<StarKind, { c: string; glow: string }> = {
...
function starClass(s?: Star): string {
  if (!s) return '—';
  const classes: Record<StarKind, string> = { sol: 'G2V', cool: 'A0V', warm: 'K2V', red: 'M4V', pale: 'B3V' };
  return classes[s.kind] || 'G0V';
}
