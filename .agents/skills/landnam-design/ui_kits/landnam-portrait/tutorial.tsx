...
function CoachAvatar({ size = 44, talking }: CoachAvatarProps) {
  return (
    <div style={{
...
function TutorialCoach({ stepIndex, steps, step, total, onManualNext, onSkip }: TutorialCoachProps) {
...
type UnlockMap = Record<PopupKind, UnlockDef>;
...
function UnlockPopup({ kind, onClose }: UnlockPopupProps) {
...
interface UnlockArtProps {
  kind: 'rocket' | 'star' | 'coin' | string;
  accent: string;
}

function UnlockArt({ kind, accent }: UnlockArtProps) {
...
function BuildGatePrompt({ onBuild, onClose, francs }: BuildGatePromptProps) {
...
window.LandnamTutorial = { M1_STEPS, TutorialCoach, CoachAvatar, UnlockPopup, BuildGatePrompt } as unknown as Window['LandnamTutorial'];
