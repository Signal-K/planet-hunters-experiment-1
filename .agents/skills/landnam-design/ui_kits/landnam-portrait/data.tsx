...
// ─── Minerals ────────────────────────────────────────────────────────
const MINERAL_META: MineralMetaMap = {
...
// ─── Contractors ─────────────────────────────────────────────────────
const CONTRACTORS: ContractorMap = {
...
function compatibleTargetsFor(mission?: Mission | null): Target[] {
  if (!mission) return [];
  const needed = Object.keys(mission.requires.minerals) as MineralId[];
...
function validateBuild({ mission, target, rocket }: ValidateBuildArgs): ValidateBuildResult {
  ...
  if (mission) {
    const cargoNeeded = Object.values(mission.requires.minerals).reduce((a: number, b) => a + (b || 0), 0);
...
function suggestBuild({ mission, target, level = 4 }: SuggestBuildArgs): RocketBuild {
  // find lowest-tier chassis with enough cargo
  const cargoMin = Math.max(
    mission ? mission.requires.cargo_min : 0,
    mission ? Object.values(mission.requires.minerals).reduce((a: number, b) => a + (b || 0), 0) : 0
  );
...
function sellCargo(cargo: Cargo): number {
  return Object.entries(cargo).reduce((a, [k, v]) => a + (MINERAL_META[k as MineralId]?.price || 0) * (v || 0), 0);
}

// Rate a mission outcome (1–3 stars)
function rateMission({ mission, cargo, elapsed }: RateMissionArgs): number {
  if (!mission) {
    // sandbox mission — rate on cargo fill
    const total = Object.values(cargo).reduce((a: number, b) => a + (b || 0), 0);
    return total > 100 ? 3 : total > 50 ? 2 : 1;
  }
  // Did we hit each mineral requirement?
  const need = mission.requires.minerals;
  let stars = 0;
  const allHit = Object.entries(need).every(([k, v]) => (cargo[k as MineralId] || 0) >= (v || 0));
  if (allHit) stars = 2;
  // Bonus: hit each by 50%+
  const surplus = Object.entries(need).every(([k, v]) => (cargo[k as MineralId] || 0) >= (v || 0) * 1.5);
