...
> `tutorial.tsx` (TutorialCoach, UnlockPopup, BuildGatePrompt, M1_STEPS) plus
> `app.tsx` (how the coach is screen-driven). All colors/sizes come from `DS.gd`.
...
2) M1_STEPS data (mirror the prototype exactly)
   ...
   Body copy: use the exact lines from tutorial.tsx M1_STEPS.
...
5) Deferred popups (reuse existing overlay scenes; trigger via AppController flags,
   ...
   Style each like UnlockPopup in tutorial.tsx: ray-burst backdrop, big art, 3 stat
...
- The exact coach copy, step anchors, spotlight regions, popup stat chips and the
  build-gate numbers all live in `ui_kits/landnam-portrait/tutorial.tsx`. Lift them
  verbatim.
- The screen-driven controller pattern (so the coach and the real buttons never
  desync) is implemented in `ui_kits/landnam-portrait/app.tsx` — read `coachAdvance`
