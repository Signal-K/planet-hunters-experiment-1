...
let capturedTickerCallback: ((t: { deltaTime: number }) => void) | null = null
let mockSceneUpdate = vi.fn()

vi.mock('pixi.js', () => ({
  Application: class {
    init = vi.fn().mockResolvedValue(undefined)
    ticker = { add: vi.fn((cb: (t: { deltaTime: number }) => void) => { capturedTickerCallback = cb }) }
    destroy = vi.fn()
  },
}))
...
describe('LaunchSequenceCanvas per-frame error isolation (SSL-281)', () => {
  ...
  it('a throwing frame force-completes the sequence instead of silently freezing forever', async () => {
    ...
    act(() => { capturedTickerCallback?.({ deltaTime: 1 }) })
