...
func buildMux(store Store, attDir string) http.Handler {
	...
	// Admin: reload JSON from disk without restart
	mux.HandleFunc("POST /api/admin/reload", reloadHandler(store))

	// Local: open native macOS file picker, returns selected path(s)
	mux.HandleFunc("GET /api/local/pick-file", filePickerHandler())
	// Local: expand ~ in a path to the home directory
	mux.HandleFunc("GET /api/local/expand", expandPathHandler())
	...
	// Static files — embedded for Vercel; local dev uses main.go's FileServer instead.
	// The "/" pattern matches anything not caught by a more specific route above.
