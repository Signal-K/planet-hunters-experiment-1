...
# Godot 4.5 on Linux stores user:// at ~/.local/share/<project-name>/
# Try explicit candidates first, then fall back to newest dir under ~/.local/share
USER_DATA_DIR=""
for candidate in \
  "${HOME}/.local/share/Landnám" \
  "${HOME}/.local/share/Landnam" \
  "${HOME}/.local/share/scene" \
  "${HOME}/.local/share/Star Sailors"; do
  if [ -d "$candidate" ]; then
...
if [ -z "$USER_DATA_DIR" ]; then
  USER_DATA_DIR=$(ls -dt "${HOME}/.local/share/"*/ 2>/dev/null \
    | grep -v "\.local/share/godot" | head -1)
fi
echo "[sandbox] User data dir: ${USER_DATA_DIR:-NOT FOUND}"
if [ -n "$USER_DATA_DIR" ] && [ -d "$USER_DATA_DIR/sandbox_screenshots" ]; then
  mkdir -p "$OUTPUT_DIR/screenshots"
  cp -r "$USER_DATA_DIR/sandbox_screenshots/." "$OUTPUT_DIR/screenshots/" 2>/dev/null || true
  N=$(ls "$OUTPUT_DIR/screenshots/"*.png 2>/dev/null | wc -l)
  echo "[sandbox] Copied $N screenshots → $OUTPUT_DIR/screenshots/"
fi
for f in sandbox_report.md sandbox_manifest.json sandbox_issues.json; do
  [ -n "$USER_DATA_DIR" ] && [ -f "$USER_DATA_DIR/$f" ] \
    && cp "$USER_DATA_DIR/$f" "$OUTPUT_DIR/" && echo "[sandbox] Copied $f" || true
done
