// Package sharedauth verifies bearer tokens issued by the shared Star
// Sailors PocketBase backend (the one players actually authenticate
// against) by calling its auth-refresh endpoint over HTTP.
//
// Ported from saily/backend/internal/sharedauth/verifier.go — kept as
// close to the original as reasonably possible so the two stay in sync.
package sharedauth
