migrate((app) => {
  ...
  upsert('clients', 'slug', [
    { slug: 'contractor-03a', name: 'Client Slot 03A', color: '#d97150', initial: '3A', unlock_tier: 3,  project_type: 'starter smelting throughput', mineral_preferences: JSON.stringify(['iron', 'silicon']), payout_notes: 'Low rate, steady volume', affinity_notes: '+10 per delivery', ui_role: 'starter' },
    { slug: 'contractor-03b', name: 'Client Slot 03B', color: '#9becff', initial: '3B', unlock_tier: 3,  project_type: 'volatile handling', mineral_preferences: JSON.stringify(['ice', 'carbon']), payout_notes: 'Medium pay, bulk orders', affinity_notes: '+8 per delivery', ui_role: 'bulk' },
    { slug: 'contractor-04a', name: 'Client Slot 04A', color: '#ffd166', initial: '4A', unlock_tier: 4,  project_type: 'precious-metal assay', mineral_preferences: JSON.stringify(['gold', 'nickel']), payout_notes: 'High pay, low volume', affinity_notes: '+15 per delivery', ui_role: 'prospect' },
    { slug: 'contractor-04b', name: 'Client Slot 04B', color: '#a8d8ea', initial: '4B', unlock_tier: 4,  project_type: 'construction aggregates', mineral_preferences: JSON.stringify(['iron', 'carbon']), payout_notes: 'Low rate, large orders', affinity_notes: '+6 per delivery', ui_role: 'bulk' },
    { slug: 'contractor-06a', name: 'Client Slot 06A', color: '#70e070', initial: '6A', unlock_tier: 6,  project_type: 'deep-core sampling', mineral_preferences: JSON.stringify(['nickel', 'cobalt']), payout_notes: 'Mixed-bag premium', affinity_notes: '+12 per delivery', ui_role: 'prospect' },
    { slug: 'contractor-06b', name: 'Client Slot 06B', color: '#c084ff', initial: '6B', unlock_tier: 6,  project_type: 'rare-gas refining', mineral_preferences: JSON.stringify(['rare', 'gold']), payout_notes: 'High tier, small batches', affinity_notes: '+20 per delivery', ui_role: 'command' },
    { slug: 'contractor-08a', name: 'Client Slot 08A', color: '#ff8c42', initial: '8A', unlock_tier: 8,  project_type: 'solar-grade silicon', mineral_preferences: JSON.stringify(['silicon', 'ice']), payout_notes: 'Premium purity contracts', affinity_notes: '+10 per delivery', ui_role: 'command' },
    { slug: 'contractor-08b', name: 'Client Slot 08B', color: '#5fcde6', initial: '8B', unlock_tier: 8,  project_type: 'outer-belt volatiles', mineral_preferences: JSON.stringify(['ice', 'carbon']), payout_notes: 'Medium rate, special cargo', affinity_notes: '+8 per delivery', ui_role: 'bulk' },
    { slug: 'contractor-10a', name: 'Client Slot 10A', color: '#f5a623', initial: '10A', unlock_tier: 10, project_type: 'strategic minerals', mineral_preferences: JSON.stringify(['gold', 'rare', 'cobalt']), payout_notes: 'High payout, rare minerals', affinity_notes: '+25 per delivery', ui_role: 'science' },
    { slug: 'contractor-10b', name: 'Client Slot 10B', color: '#39d36a', initial: '10B', unlock_tier: 10, project_type: 'base expansion reserves', mineral_preferences: JSON.stringify(['iron', 'silicon', 'nickel']), payout_notes: 'Balanced late-game contracts', affinity_notes: '+12 per delivery', ui_role: 'starter' },
  ])
  ...
  upsert('mission_templates', 'slug', [
    { slug: 'starter-bulk', tag: 'STARTER', difficulty: 'L1', mineral_keys: JSON.stringify(['iron', 'silicon', 'carbon']), cargo_min: 4, cargo_max: 8, drill_tier_min: 1, orbit_max: 4, payout_multiplier: 1.0, client_role: 'starter', payout_formula: 'mineral price * amount * 1500 * multiplier' },
    { slug: 'volatile-bulk', tag: 'BULK', difficulty: 'L2', mineral_keys: JSON.stringify(['ice', 'carbon', 'silicon']), cargo_min: 8, cargo_max: 14, drill_tier_min: 1, orbit_max: 5, payout_multiplier: 1.35, client_role: 'bulk', payout_formula: 'mineral price * amount * 1500 * multiplier' },
    { slug: 'metal-prospect', tag: 'PROSPECT', difficulty: 'L2', mineral_keys: JSON.stringify(['nickel', 'cobalt', 'gold']), cargo_min: 3, cargo_max: 8, drill_tier_min: 2, orbit_max: 5, payout_multiplier: 2.25, client_role: 'prospect', payout_formula: 'mineral price * amount * 1500 * multiplier' },
    { slug: 'command-reserve', tag: 'COMMAND', difficulty: 'L3', mineral_keys: JSON.stringify(['gold', 'rare', 'cobalt']), cargo_min: 3, cargo_max: 6, drill_tier_min: 2, orbit_max: 6, payout_multiplier: 3.5, client_role: 'command', payout_formula: 'mineral price * amount * 1500 * multiplier' },
  ])

  upsert('missions_catalog', 'slug', [
    { slug: 'm1-iron', title: 'Iron Reserve Order', client_slug: 'contractor-03a', brief: 'Client Slot 03A needs a starter iron shipment from a reachable asteroid.', tag: 'STARTER', difficulty: 'L1', locked: false, sequence: 1, unlock_at: '', requires_minerals: JSON.stringify({ iron: 6 }), requires_cargo_min: 6, requires_drill_tier: 1, requires_max_orbit: 4, payout_francs: 1080000, payout_xp: 108, payout_affinity: 10 },
    { slug: 'm2-silicon', title: 'Silicon Bulk Order', client_slug: 'contractor-03b', brief: 'Client Slot 03B needs raw silicon for electronics-grade supply contracts.', tag: 'BULK', difficulty: 'L2', locked: false, sequence: 2, unlock_at: 'Complete M1', requires_minerals: JSON.stringify({ silicon: 8 }), requires_cargo_min: 8, requires_drill_tier: 1, requires_max_orbit: 5, payout_francs: 2916000, payout_xp: 291.6, payout_affinity: 8 },
  ])
}, (app) => {
  const slugs = {
    ...
    clients: ['contractor-03a', 'contractor-03b', 'contractor-04a', 'contractor-04b', 'contractor-06a', 'contractor-06b', 'contractor-08a', 'contractor-08b', 'contractor-10a', 'contractor-10b'],
