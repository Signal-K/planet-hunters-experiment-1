...
##
## Tests bidirectional state synchronization between React Native and Godot.
## Verifies:
## - State initialization from React Native
## - Godot state changes propagate correctly
## - React Native state changes apply to AppController
...
func test_get_state_returns_all_keys() -> void:
	...
	var expected_keys = ["counter", "tutorialCompleted", "francBalance", "experienceXp", "experienceLevel"]
...
func test_set_value_updates_state() -> void:
	...
	sync.set_value("tutorialCompleted", true)
	state = sync.get_state()
	
	if state.get("tutorialCompleted") != true:
		reporter.fail_test("Expected tutorialCompleted=true")
		await _teardown(ctx["app"], sync)
		return
...
func test_app_controller_changes_update_sync_bridge() -> void:
	...
	# Change via AppController directly (simulating game action)
...
func test_init_from_react_sets_all_values() -> void:
	...
	if state.get("tutorialCompleted") != true:
		reporter.fail_test("tutorialCompleted should be true")
		await _teardown(app, sync)
		return
	...
	# Verify AppController was also updated
...
func test_no_feedback_loop() -> void:
	...
	# Setting from React should emit exactly one signal with source "react"
	sync.set_value("counter", 77)
	await create_timer(0.1).timeout
	
	# Filter for just the counter signal from react
	...
	# Ensure the signal came from react, not godot (no feedback loop)
