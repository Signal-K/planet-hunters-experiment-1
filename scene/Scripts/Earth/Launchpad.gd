...
func _ready():
	...
	Logger.d("Launchpad initialized: %s" % structure_name)
	set_process_unhandled_input(true)
	
	# Debug: List all children at startup
	Logger.d("Launchpad children at startup:")
	for child in get_children():
		Logger.d("  - %s (%s)" % [child.name, child.get_class()])
	...
	if app_controller and app_controller.has_signal("rockets_reset"):
		...
		Logger.d("Launchpad: connected to AppController rockets_reset signal")
	else:
		Logger.w("Launchpad: AppController not found or missing rockets_reset signal")
	...
	if not scene_manager:
		...
		for child in main_scene.get_children():
			if child is SceneManager:
				scene_manager = child
				break
		Logger.d("Found SceneManager as child: %s" % (scene_manager != null))
	
	if scene_manager:
		Logger.d("Transitioning to earth_launchpad scene...")
		scene_manager.change_to_scene("res://Scenes/Earth/earth_launchpad.tscn")
	else:
		Logger.w("Launchpad: SceneManager not found, using direct scene change fallback")
...
func _on_rockets_reset() -> void:
	Logger.d("Launchpad: rockets reset event received, clearing rockets")
	clear_rockets()
...
func clear_rockets() -> void:
	...
	for n in nodes:
		if is_instance_valid(n):
			n.queue_free()
	Logger.d("Launchpad: cleared rockets from scene")
