...
# Drawing canvas for annotating asteroid images
# Supports freeform strokes, rectangles, and circles
...
func _draw():
	...
	for s in all_strokes:
		...
		if t == "freeform":
			...
			draw_rect(r, Color(c.r, c.g, c.b, 0.0), false)
	...
	if is_drawing:
		if mode == Mode.FREEFORM:
			# last stroke is being appended to; draw its points
