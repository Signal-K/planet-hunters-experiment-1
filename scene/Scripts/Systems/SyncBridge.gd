...
## Unified Sync Bridge
## 
## Handles bidirectional state synchronization between React Native and Godot.
## 
## Architecture:
## - Godot is authoritative for game state during gameplay
## - React Native handles persistence (AsyncStorage)
## - All updates are event-driven (no version conflicts)
## - Simple getter/setter pattern for cross-platform calls
...
# Track if we're applying updates from React to prevent feedback loops
...
func _connect_to_app_controller() -> void:
	...
	# Load initial state from AppController
	_state["counter"] = app.get_counter() if app.has_method("get_counter") else 0
	_state["tutorialCompleted"] = app.get_tutorial_completed() if app.has_method("get_tutorial_completed") else false
	...
	# Connect to AppController signals to track Godot-side changes
	...
	if app.has_signal("tutorial_completed_updated"):
		app.tutorial_completed_updated.connect(_on_tutorial_updated)
...
# ============================================================================
# React Native API - Called from React Native via worklets
# ============================================================================

## Get current state snapshot for React Native
...
## Set multiple values from React Native
...
## Initialize state from React Native (called on game start)
...
# ============================================================================
# Apply Updates to AppController
# ============================================================================
...
func _apply_to_app_controller(key: String, value: Variant) -> void:
	...
	match key:
		...
		"tutorialCompleted":
			if app.has_method("set_tutorial_completed_from_react"):
				app.set_tutorial_completed_from_react(bool(value))
		"francBalance":
...
# ============================================================================
# Handle AppController Updates (Godot-side changes)
# ============================================================================
...
func _on_tutorial_updated(value: bool) -> void:
	if _applying_from_react:
		return
	_state["tutorialCompleted"] = value
	state_changed.emit("tutorialCompleted", value, "godot")
